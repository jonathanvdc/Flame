// Based on the .NET Compiler Platform (Roslyn) CryptoBlobParser.
// https://github.com/dotnet/roslyn/blob/master/src/Compilers/Core/Portable/StrongName/CryptoBlobParser.cs
...
namespace Flame.Front.AssemblyResolution
{
    internal static class CryptoBlobParser
    {
        ...
        // From ECMAKey.h
        private static readonly IReadOnlyList<byte> s_ecmaKey = new byte[] { 0, 0, 0, 0, 0, 0, 0, 0, 4, 0, 0, 0, 0, 0, 0, 0 };
        ...
        private static uint ToUInt32(IReadOnlyList<byte> bytes, int offset)
        {
            Debug.Assert((bytes.Count - offset) > sizeof(int));
        ...
        internal static bool IsValidPublicKey(IReadOnlyList<byte> blob)
        {
            // The number of public key bytes must be at least large enough for the header and one byte of data.
            if (blob == null || blob.Count < s_publicKeyHeaderSize + 1)
            {
                return false;
            }

            return IsValidPublicKeyUnsafe(blob.ToArray());
        }

        private static unsafe bool IsValidPublicKeyUnsafe(byte[] blob)
        {
        ...
        private static IReadOnlyList<byte> CreateSnPublicKeyBlob(BlobHeader header, RsaPubKey rsa, byte[] pubKeyData)
        {
        ...
        private unsafe static bool TryGetPublicKeyFromPublicKeyBlob(byte* blob, int blobLen, out IReadOnlyList<byte> publicKey)
        {
        ...
        private unsafe static bool TryGetPublicKeyFromPrivateKeyBlob(byte* blob, int blobLen, out IReadOnlyList<byte> publicKey)
        {
        ...
        /// </remarks>
        public unsafe static bool TryGetPublicKey(byte[] blob, out IReadOnlyList<byte> publicKey)
        {
            publicKey = new byte[] { };
